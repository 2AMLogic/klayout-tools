...
| `magic-tech/` | `sky130A.tech` / `gf180mcuC.tech`, generated by `scripts/fetch-magic-tech.sh` from a pinned, checksum-verified open_pdks commit — the technology decks the magic cross-validation oracle runs against (issue #2014). Not a PDK install: three small source files put through open_pdks' own `preproc.py`, so the oracle needs no multi-GB PDK. See [`docs/design/magic-oracle.md`](../docs/design/magic-oracle.md) |
