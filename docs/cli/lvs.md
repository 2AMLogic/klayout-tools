...
- **Resistor and capacitor** convert to plain `R`/`C` cards the same way —
  ...
  The class's `L`/`W` (resp. `A`/`P`) geometry is *also* outside the compare,
  since KLayout declares it secondary — so nothing about such a device is
  compared and the pairing rests on topology alone, disclosed as a companion
  `device.geometry_not_compared` entry (issue #2461).
...
**A cell with *only* power/ground pins is pruned automatically from
whichever side instantiates it** (issue #1622, extended symmetrically to
the reference side by issue #2244) — a filler cell
(`sky130_fd_sc_hd__fill_*`, inserted unconditionally whenever issue #1442's
row-rail fallback fires), a tap cell (`sky130_fd_sc_hd__tapvpwrvgnd_1`,
inserted unconditionally by the `tapcell` stage), or an endcap carries no
logic function, so `klt place-and-route`'s own `verilog_path` writer never
...
the cell's *library-declared* pins is a power/ground pin, derived from the
library's own `.subckt` data rather than a cell-name glob like
`fill_*`/`tap*` or a hardcoded PDK power-pin table — and removes the
instance from every side that has one before comparing, disclosing what it
removed as a `severity: "warning"`, `category: "topology.power_only_pruned"`
entry — see "`topology.power_only_pruned`" below — rather than reporting a
false `topology` mismatch. You do not need to exclude these cells from
`--abstract-cells` yourself.

**Not every `write_verilog` consumer honors the "never instantiates a
power-only cell" assumption above.** A gate-level netlist produced by
reading a routed DEF back through `write_verilog` (as opposed to `klt
place-and-route`'s own `verilog_path` writer) can carry a filler/tap/endcap
instantiation line per placed physical-only component, each with an empty
connection list (`library__fill_4 FILLER_0_10 ();`), because the cell has
no signal pins to connect. `klt lvs` detects this the same structural way —
by the *library's* full declared pin order for that cell type, not by what
the reference itself happens to connect (which can be nothing at all for
this shape) — and prunes it from the reference side too. See
"`topology.power_only_pruned`" below for the full symmetric-prune contract.
...
Issue #1622's `_prune_power_only_circuits` removes every circuit — layout
side, reference side (issue #2244), or both — whose entire pin list is
power/ground — fillers, taps, decaps — because a signal-only compare has
nothing to say about them (see "`topology.power_only_pruned`" below). This
check runs **before** that prune, deliberately, and only ever inspects the
*layout* side's extracted connectivity (the reference carries no power
nets to check either way): those cells are precisely the ones whose power
...
| `device_parameter_coverage` | array \| `null` | Issue #2461. Per participating **resistor/capacitor** device class, which of its declared parameters this compare actually covered — the machine-checkable form of the `device.geometry_not_compared` warning (see below), reported beside `status` rather than folded into it. One object per class, sorted by class name: `class` (the device class's own name), `device_kind` (`"resistor"` \| `"capacitor"`), `layout_devices` / `reference_devices` (how many instances each side contributed), `compared` (the parameters `NetlistComparer` compared, in the class's own declaration order), and `not_compared` (one `{"parameter": <string>, "reason": <string>}` object per parameter that took no part, with `reason` one of `"secondary"` — KLayout's own class definition marks it non-primary, the case with no other disclosure anywhere in the report — `"placeholder_value"` or `"compare_parameters"`, naming the request-side hook that excluded it, each of which also has its own `mismatches[]` entry). `[]` when no resistor/capacitor class participated (a MOS class's own `L`/`W` are *primary* and therefore compared, so it is deliberately not reported here), and `null` for `"engine": "netgen"`, which builds no KLayout device classes to read this from. This field never changes `status`, `mismatch_count`, `error_count`, `category_counts` or `category_error_counts` — answering "was this class's sizing verified?" is exactly its purpose, and the answer is routinely "no" on a perfectly good `"match"`. |
...
#### `device.class_family_unsatisfiable`: the reference mixes classes one deck option selects between

Some PDKs extract several *named* device classes from **one drawn layer**, and
a whole-run deck option picks which name (and sheet resistance / capacitance
density) that layer extracts as. gf180mcu's high-sheet-rho poly resistors are
the worked example: `ppolyf_u_1k`, `ppolyf_u_2k` and `ppolyf_u_3k` are the
*identical* drawn `Resistor`-marked poly segment, and the deck's `poly_res`
option — set by the wafer's implant option, not by anything drawn per instance
— decides which single one every instance on that layer extracts as (see
`--deck-option` / `layout.deck_options` above, and issue #595). Its siblings
are gf180mcu's `mim_cap` (MiM density) and `metal_top` (top-metal thickness)
families; the check below is derived from the deck's own data
(`ResistorDevice.flavour_option`/`CapacitorDevice.flavour_option` plus
`flavours`), so any PDK family of that shape is covered with no further code.

A reference netlist that instantiates **two or more** classes of such a family
therefore cannot pass LVS under *any* option value: whichever one is selected,
the instances of the other class(es) are compared against a device class the
extraction never produces. Before issue #2421 that surfaced only as an ordinary
`device.class`/`device.unmatched` mismatch, identical for every option value —
so the obvious next step ("try the other value") reproduced the same failure,
and only reading the PDK deck's own derivation rules revealed that the classes
share one drawn layer.

`klt lvs` now runs this as a **pre-flight** check — on the reference netlist as
given, against the deck's family table, independent of which `deck_options`
value the run itself selected — and reports one `category:
"device.class_family_unsatisfiable"`, `severity: "error"`, `side: "reference"`
entry per affected family:

```json
{
  "category": "device.class_family_unsatisfiable",
  "severity": "error",
  "description": "the reference netlist instantiates 2 device classes of the deck's 'poly_res' resistor family (PPOLYF_U_1K, PPOLYF_U_3K) -- that family's members (ppolyf_u_1k, ppolyf_u_2k, ppolyf_u_3k) are all extracted from the same drawn geometry, and the deck option 'poly_res' selects which single one every instance on that layer extracts as, for the whole run. No value of that option can therefore match this reference: whichever is selected, the instances of the other class(es) are compared against a device class the extraction never produces. The design has to commit to one class (one process option) for the whole run (see docs/cli/lvs.md, 'device.class_family_unsatisfiable')",
  "side": "reference",
  "net": null,
  "device": null,
  "property": null,
  "details": {
    ...
    "family": ["ppolyf_u_1k", "ppolyf_u_2k", "ppolyf_u_3k"],
    "reference_classes": ["PPOLYF_U_1K", "PPOLYF_U_3K"]
  }
}
```

Like `device.class_arity` above, the entry is **diagnostic**: it explains the
mismatch the compare already reported and never moves `status` on its own, and
the collateral `device.unmatched`/`net.unmatched`/`topology` entries the
comparer produced for the same devices are still reported alongside it. The
fix is on the design/reference side, not in the request: the design has to
commit to **one** class of the family (one process option) for the whole run,
and select it with `layout.deck_options` (`--deck-option poly_res=3k`).

Scope and non-triggers:

- Requires a resolved `layout.deck` — the deck is where the family table lives.
  The pre-extracted `layout.netlist` shape with no `layout.deck` reports
  nothing. Both engines and both layout shapes are otherwise covered, since the
  check reads only the reference netlist and the deck.
- Counts only classes with at least one **instantiated** reference-side device,
  so a class a reader or deck merely registers cannot raise a false positive.
- Matched case-insensitively (`NetlistSpiceReader` reads `ppolyf_u_1k` back as
  `PPOLYF_U_1K`); `details.reference_classes` echoes the reference's own
  spelling, `details.family` the deck's.
- A reference naming exactly **one** class of a family is the normal, supported
  case and reports nothing — that is precisely what `deck_options` is for.
- Geometrically distinguishable siblings are unaffected: the base `ppolyf_u`
  and the `Resistor`-marked `ppolyf_u_1k` are separate deck entries told apart
  by drawn layers, not by an option, so a reference naming both is satisfiable
  and silent here.
...
- **Only the value parameter is excluded**, not the class's geometry. `L`/`W`
  ...
  checked exactly as before. **Note what that means, though**: because those
  geometry parameters are never compared *and* the value is excluded here,
  no dimension of the device is compared at all and the two sides are paired
  on topology alone. Issue #2461 added a companion
  `device.geometry_not_compared` warning (and the `device_parameter_coverage`
  block) that says so in as many words — reading this entry on its own used
  to suggest that only the value had gone unchecked.
...
#### `device.geometry_not_compared`: a resistor/capacitor class's geometry took no part in the compare

Issue #2461, `"engine": "klayout"` only. KLayout's `DeviceClassResistor`
declares `R`, `L`, `W`, `A`, `P` and marks **only `R`** primary;
`DeviceClassCapacitor` declares `C`, `A`, `P` and marks **only `C`** primary.
`NetlistComparer` compares primary parameters, so for either class the drawn
device's *entire geometry* sits outside every default compare — a reference
resistor whose `W` is an order of magnitude off the layout's still reports
`status: "match"` with `error_count: 0`. This is correct behaviour for what
was checked; the problem this entry fixes is that nothing in the report used
to say so.

It is a **disclosure, not a finding**: one `severity: "warning"`,
`side: "both"` entry per participating class per run (never per instance),
emitted for every `reference.form` — it is a property of the device class's
own parameter definitions, not of these two netlists.

```json
{
  "category": "device.geometry_not_compared",
  "severity": "warning",
  "description": "device class 'RES_XHIGH_PO' is a resistor class, whose ['A', 'L', 'P', 'W'] parameter(s) KLayout declares *secondary*: kdb.NetlistComparer compares primary parameters only, so none of them took part in this compare and each is NOT verified -- this resistor's geometry could differ between the two sides by any amount and this run would still report 'match'. The compared parameter(s) were ['R']. Name them in options.compare_parameters to compare them (see docs/cli/lvs.md, 'device.geometry_not_compared')",
  "side": "both",
  "net": null,
  "device": {"layout": null, "reference": null, "class": "RES_XHIGH_PO"},
  "property": null,
  "details": {
    "device_kind": "resistor",
    "compared_parameters": ["R"],
    "not_compared_parameters": ["A", "L", "P", "W"],
    "layout_devices": 4,
    "reference_devices": 4
  }
}
...
- **It never changes `status`.** Always `severity: "warning"`, like every
  other disclosure in this family — a run whose only finding is this entry
  reports `status: "match"` with a nonzero `mismatch_count`.
- **Under `reference.form: "subckt-call"` it is the other half of
  `device.placeholder_value`.** There the primary `R`/`C` is *also* excluded
  (a literal `0` placeholder — see above), so **no** parameter of the class
  is compared at all and the two sides are paired on topology alone. The
  description says so explicitly in that case (`compared_parameters: []`),
  which is the gap the issue was filed from: a reader seeing only
  `device.placeholder_value` could reasonably conclude that the *value* had
  gone unchecked but the geometry had been compared.
- **It reads the compare that is actually about to run, not the class's
  defaults.** `options.compare_parameters`' `enable_parameter()` flips
  `is_primary` in place, so a class scoped to include `W` reports `W` under
  `compared` and never claims it went uncompared.
- **It never duplicates another disclosure.** A parameter excluded by
  `options.compare_parameters` (`device.parameter_excluded`) or by the
  `subckt-call` placeholder path (`device.placeholder_value`) is reported by
  *that* entry, and appears here only in the `device_parameter_coverage`
  block with the matching `reason`.
- **Only resistors and capacitors.** A MOS class's `L`/`W` are primary and
  compared; its secondary `AS`/`AD`/`PS`/`PD` diffusion parameters are a much
  narrower gap, and disclosing them would put a warning on essentially every
  `klt lvs` run without telling a reader anything new about whether the
  device was *sized* correctly. A custom device class whose geometry
  parameters are primary (e.g. the MoM capacitor's `W`/`L`, see
  `extract.mom_capacitor_device_class`) is likewise not reported — its
  geometry really is compared.
- **To actually compare the geometry**, name the parameters in
  `options.compare_parameters` for that class — e.g.
  `{"RES_XHIGH_PO": ["R", "L", "W"]}`. Both sides must state them for that to
  be meaningful; under `subckt-call` the reference's `R` is a placeholder, so
  scope to the geometry alone (`["L", "W"]`) rather than including `R`.

##### The same condition, machine-checkable: `device_parameter_coverage`

A `mismatches[]` warning is not gradeable — a downstream consumer had to
string-match a category inside an array of ordinary compare findings to ask
"was this class's sizing verified?", so in practice nothing asked. The
top-level **`device_parameter_coverage`** block (always present; `null` only
for `"engine": "netgen"`) answers it directly, built from the same
determination as the warning so the two cannot disagree — the same
one-source-of-truth discipline `body_verification` (issue #1983) and
`power_connectivity` (issue #1952) already apply:

```json
"device_parameter_coverage": [
  {
    "class": "RES_XHIGH_PO",
    "device_kind": "resistor",
    "layout_devices": 4,
...
A consumer grading an `lvs` envelope as signoff evidence can assert
`compared` contains the dimensions it cares about — `all(c["compared"] for c
in report["device_parameter_coverage"])` rejects exactly the
`subckt-call`-placeholder shape above, where nothing about the device was
compared at all.
...
#### `device.combine_parameter_corrected`: `options.combine_devices` produced a combined device with wrong parameters and they were corrected

Only possible when `options.combine_devices: true` (issues #1497, #2374). This
is a **distinct failure mode** from `device.combine_incomplete` above: that
...
opposite shape: `combine_devices()` returns **normally** and the device
topology folds correctly (exactly one device remains per parallel group, with
the expected terminal connectivity), but the surviving device's *parameters*
are inconsistent with the group it absorbed. No exception is raised, so the
existing `device.combine_incomplete` retry mitigation
(`options.combine_devices_max_attempts`) has no signal to act on: the call
simply "succeeds" with wrong numbers.

Two shapes have been reported:

- **Capacitor `C` (issue #1497)** — the group's secondary `A` (area) and `P`
  (perimeter) parameters are correctly summed, but the primary `C`
  (capacitance) parameter is sometimes left at a single pre-combine
  instance's own value instead of the group's summed total.
- **Bipolar array inversion (issue #2374)** — a parallel
  `DeviceClassBJT3Transistor`/`DeviceClassBJT4Transistor` array's whole
  accumulation comes back *inverted*: `AE` and `NE` (which the correct fold
  sums across the array) left at a single instance's value, and `AB`/`PB`/
  `AC`/`PC` (which the correct fold leaves at the array's shared value,
  because they describe the base and collector regions the instances have in
  common) summed instead. The device *count* is correct either way, so
  `counts.devices` looks right and only `device.property` findings expose it
  — which is exactly what made it a false `status: "mismatch"` rather than a
  visible failure.

Both are caught by the same **post-combine parameter-conservation check**.
Every group of parallel devices sharing one (circuit, device class,
terminal-connectivity) identity has a known post-combine parameter set given
only its pre-combine members: a *summed* parameter (a capacitor's `C`; a
bipolar's `AE`/`PE`/`NE`) is a total `combine_devices()` can only redistribute
across fewer surviving devices, never change, and a *preserved* parameter (a
bipolar's `AB`/`PB`/`AC`/`PC`) stays at the group's single shared value. `klt
lvs` checks those invariants directly: it snapshots each side's netlist before
`combine_devices()` runs (the same snapshot issue #1370's symmetric degrade
already takes), and after a successful combine compares every resulting
device's governed parameters against its own pre-combine group. Wherever they
disagree beyond floating-point tolerance, `klt lvs` overwrites the parameter
**in place** with the group's correct value and records one
`severity: "warning"` `device.combine_parameter_corrected` entry per affected
side, naming every corrected `(device, parameter)` pair and its before/after
values — so a caller relying on the combined value can see that a correction
happened rather than silently trusting a value KLayout itself got wrong.

The check is deliberately narrow in two directions. It covers only device
classes whose every combination is a parallel fold with a known accumulation
rule — capacitors and bipolars; MOS transistors (which also fold in *series*,
producing a surviving device with connectivity no pre-combine group shares)
and resistors (whose parallel combination is `1/R = 1/R1 + 1/R2`, not a sum)
are never inspected. And within those classes it only asserts against a group
that actually folded to a **single** surviving device: a group left uncombined
— e.g. because a list-valued `options.combine_devices` restricted combining
away from its class — keeps every member under the same key, and asserting a
group *total* against each member individually would corrupt correct values
rather than repair wrong ones. A preserved parameter whose pre-combine group
members do not all agree on one value is likewise skipped: nothing is known
about the folded device's value for it.
...
on every `options.combine_devices: true` request with capacitor or bipolar
devices,
...
**The `dup()`-copy exposure behind the #2374 shape, and why it is no longer on
the common path.** Issue #2374 measured the bipolar inversion on ~24% of
`Netlist.dup()` copies of one array and on 0/1000 combines run against the
netlist itself — consistent with the same heap-address-ordered candidate
grouping documented for `device.combine_incomplete` above: a freshly allocated
copy walks its nets in an order unrelated to the order they were built in.
Because issue #1185's retry mitigation originally ran its *first* attempt
against a copy, the exposed ordering had become the normal one, and a
`combine_devices: true` request could report `status: "match"` on ~76% of runs
and a false `status: "mismatch"` (a dozen `device.property` findings) on the
rest, against byte-identical inputs. Since issue #2374 the adopted combine
runs against the netlist itself and only the retries — reached solely when
that first attempt raised the `RuntimeError` above — resample against
independent copies of a pre-combine snapshot. The conservation check applies
to every successful combine either way, so a retry-path result is covered too.
`options.combine_devices_max_attempts: 1` (which disables retrying altogether)
is therefore no longer needed as a workaround for this shape.
...
#### `topology.power_only_pruned`: a power-only circuit was removed before comparing

Only possible when `reference.form: "gate-level-verilog"` (issue #1622), and
only emitted when at least one circuit — layout-side, reference-side, or
both (issue #2244) — declares nothing but power/ground pins.

A `klt place-and-route` `verilog_path` reference conversion never
instantiates a cell with no logic function — a filler cell
(`sky130_fd_sc_hd__fill_*`, inserted unconditionally whenever issue #1442's
row-rail fallback fires) or a tap cell (`sky130_fd_sc_hd__tapvpwrvgnd_1`,
inserted unconditionally by `klt place-and-route`'s `tapcell` stage) has
nothing on that reference to describe it, so it only ever needs pruning from
the layout side. But not every `reference.form: "gate-level-verilog"`
reference comes from that writer: a gate-level netlist produced by reading a
routed DEF back through `write_verilog` (any other DEF-derived netlist
writer) instantiates a filler/tap/endcap master too, with an **empty**
connection list per instance (`library__fill_4 FILLER_0_10 ();`, since the
cell has no signal pins to connect) — which converts to a reference-side
`.SUBCKT` stub declaring **zero** pins. Left unpruned, that stub and its
instances have no counterpart on the layout side once the layout-side prune
removes its own copy of the same master, and each surfaces as a `topology`
"circuit could not be matched to a counterpart" error — one per reference
type, one per reference instance — stranded around an otherwise-clean
`power_connectivity: "match"` verdict, because the power/ground half of the
compare runs (and passes) before this prune (see "Checked before
filler/tap pruning, not after" above).

`klt lvs` removes every power-only circuit, along with every subcircuit
instance of it, from **every side that instantiates it** before the compare
runs — the layout side alone (the common case, e.g. against a `klt
place-and-route` `verilog_path` reference), the reference side alone (a
DEF-derived reference whose layout counterpart was already abstracted
without that master), or both. A circuit with even one pin not established
as power/ground is never pruned, even if it also has power pins (see
"Negative controls" above for this module's general discipline on not
...
**The reference-side check is structural in a different way than the
layout-side one, and that difference is load-bearing.** A layout circuit is
classified by its own declared pins (the real PDK pin order a `klt extract
--abstract-cells` abstraction carries). A reference circuit cannot be
classified that way: its own declared pin list is only whichever pins some
instance actually connects, which for a DEF-derived filler/tap/endcap
instance is **empty** — zero declared pins is exactly what this module
treats elsewhere as "no evidence, do not prune" (the same discipline that
protects a genuinely unresolvable circuit from being masked). So the
reference-side check instead looks up the *library's* full declared pin
order for the matching cell name and asks whether that (non-empty) set is
entirely power/ground pins — independent of what the reference itself
happened to connect for that particular instance.
...
carry a power pin's *connection* (whether or not it emits the pin itself)
and which is the only form with a resolved `reference.library` behind it; a
`"plain-element"`/`"subckt-call"` reference is arbitrary SPICE with no
library to derive anything from, so the same inference is never made there.
Every removal is named in this entry's `description`, so a `"match"` that
depended on one is always auditable from the report alone.
...
`status: "match"` with a nonzero `mismatch_count`). `side` is `"layout"`
when only layout-side circuits were removed, `"reference"` when only
reference-side circuits were removed, and `"both"` when a run removed
circuits from each side — always exactly **one** entry per run regardless of
how many circuits were removed or from which side(s). `description` names
every circuit removed, grouped by side when both sides contributed. Present
for the same reason `topology.flattened` is: a `"match"` reached after this
pruning is never silently indistinguishable from one reached against either
...
the other side's while one side has an extra instance the other cannot
