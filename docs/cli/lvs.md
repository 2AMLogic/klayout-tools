...
| `hints.same_nets` | array\<[string, string]\> | Optional `[layout_net_name, reference_net_name]` pairs — ties a named net in the layout's top circuit to a named net in the reference's top circuit. A merged-label net may be named either the way every response field spells it (`"VPWR|VDD"`) or the way KLayout's raw API does (`"VPWR,VDD"`), so a name read out of `net_correspondence[]` can be fed straight back in (issue #696). A name that resolves under neither spelling on the stated side is an application error (exit 1), not a silent no-op. |
...
| `net` | object \| `null` | `{"layout": <name\|null>, "reference": <name\|null>}` when a net is involved. Each name is spelled exactly as the written netlist spells it, matching `net_correspondence[]` and `klt extract`'s `nets[].name` (issue #696). |
...
| Field | Type | Description |
| ----- | ---- | ----------- |
| `layout` | string | The layout net's name, spelled exactly as the written netlist spells it — the same convention `mismatches[].net` uses, so a net two drawn labels merged carries both aliases joined, e.g. `"VPWR|VDD"`. Byte-identical to that net's `klt extract` `nets[].name`/`merged_net_labels[].net` and to its node token in the `.spice` file, so the artifacts can be joined by name (issue #696 — see `klt extract`'s "Merged net labels" section for the one place this differs from KLayout's raw `Net.expanded_name()`). |
