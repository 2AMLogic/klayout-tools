...
| `connectivity[].legs[].layer_role`/`.width_um` | string / number | Optional (issue #1655). The per-*leg* tier of the two fields above: the plane (and width) **this one named leg** draws on, overriding the net's own. Resolved, floored, and exempted from the cross-block fallback exactly as the net-level fields are. This is how a single net's legs span two planes; they stitch together at the leg's own endpoint pins through the existing #454 via-drop, which already drops every leg to each pin's own reported layer. |
