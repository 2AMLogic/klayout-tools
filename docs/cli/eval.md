...
| `exit_code` | integer | The exit code the cited `check` would itself have returned for this report (e.g. `3` for a `drc` gate with violations) — `0` for a threshold-derived gate (the underlying check itself ran and reported successfully; the threshold comparison, not that check's own exit-code vocabulary, produced `status`). Lets a human debugging a `valid: false` run trace it back to the specific `klt <check>` invocation and outcome. A `4` means the check ran but could not reach a trustworthy verdict, so `status: "fail"` there is "did not clear the gate", not "the design is bad": `sim`'s broken-corner `status: "error"`, (issue #1370) an `lvs` gate whose report is `status: "inconclusive"` — `options.combine_devices` exhausted its retry budget, so the compare never ran — and (issues #2492/#2493) a `sim` gate whose report is `status: "inconclusive"`, where the sweep produced numbers it will not vouch for: a corner whose solve emitted a code listed in `options.fail_on_diagnostic`, or a measurement outside its declared `options.node_voltage_bounds`/`measurements[].plausible_range` that was therefore never graded against `limits` at all (see [`sim.md`](sim.md)'s "Grading a recovered diagnostic as inconclusive" and "Plausibility bounds"). All three are distinguished from `3` deliberately; `3` alone would claim the design differs or missed a limit. |
| `count` | integer \| number | Present when the underlying check (or threshold) has a natural headline count/value: `violation_count` for `drc`, `mismatch_count` for `lvs`, failed corner count for `sim` (or, on the exit-`4` branch, its errored **plus** `inconclusive` corner count — issues #2492/#2493, so a sweep whose only untrustworthy corners were graded `inconclusive` is not cited with a headline count of zero), `finding_count` for `op-sanity`, the threshold's extracted metric value for a threshold-derived gate. Absent otherwise. |
