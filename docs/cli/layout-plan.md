...
| `rows[].margin_um` | number | no (default: auto — see below) | The vertical gap between this row and the previous row (ignored for the first row). Must be `>= 0` when set. Omit to let plan execution (Phase C) derive it automatically as the larger of the two adjacent rows' groups' own declared `drc_hints.min_spacing_um`, so a default multi-row stack no longer places vertically-adjacent groups at exactly `0.00um` separation (issue #1170) — see [`layout-plan-execute.md`](layout-plan-execute.md)'s "Row-of-rows placement and abutment". An explicit value, including `0.0`, always overrides that default. |
