...
**PORT-less-pin cross-check (issue #464):** a macro LEF pin that `klt
lef-abstract` emitted with no `PORT` geometry (a pin whose declared layer is
not a routing-type tech-LEF layer — e.g. a device gate pin on bare poly, see
[`klt lef-abstract`'s "Pins"](lef-abstract.md#pins) section) would otherwise
only surface once OpenROAD's global router hits it mid-`route`, as an opaque
`GRT-0029` error several stages into a real run (discovered during Epic #393
Phase 3, #456; full repro in #464). `request.macros` validation closes that
gap: for each declared macro, it reads the LEF's own `PIN` list (already
needed to resolve `cell_name`) and best-effort scans the netlist's own
Verilog text for that instance's **named** port connections (`.PIN(NET)`).
A pin the netlist actually wires to a non-empty net, but whose LEF entry has
no `PORT` at all, is rejected here — before any OpenROAD subprocess runs —
with a clear error naming the instance, pin, and LEF path. A pin the
netlist never wires (left `.PIN()` open, or not mentioned at all) is never
flagged, and a positional (non-named) instantiation style this scan cannot
confidently resolve is silently skipped rather than guessed — a missed
detection here is safe (falls back to OpenROAD's own `GRT-0029`), a
false-positive reject is not.
