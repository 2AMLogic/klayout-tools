...
## Update (issue #389): trigger fired, resolved without scgallery/zerosoc

The first "Revisit when" condition above has since fired — `klt drc`
(Phase 2) and `klt lvs`/`klt extract` (Phase 4) all shipped — and the need
it named (a DRC/LVS-scale oracle fixture) was **resolved via a different,
better path than either repo surveyed here**. Rather than vendoring or
building from scgallery/zerosoc, the fixture is now generated by klt's *own*
digital flow: `klt synthesize` (real Yosys) → `klt place-and-route` (real
OpenROAD) produces a real `sky130_fd_sc_hd` GCD macro (thousands of
instances, `met1`-`met5` routing), committed as
`tests/corpus/place_and_route/gcd.gds.gz` with a regeneration recipe
(`regenerate.sh`) and full provenance (`tests/corpus/README.md`). This
landed as Phase 5 of [Epic #391](https://github.com/2AMLogic/klayout-tools/issues/391)
(PRs #431/#443), with `klt lvs` verification added by issue #389.

This closes out the deferral: the oracle-fixture need is met, and it is met
by a *regenerable, in-repo-toolchain* GDS rather than an opaque or
out-of-band one — precisely the "regenerable rather than a mystery blob"
concern this survey raised. The one caveat: the klt-native fixture has a
single level of real hierarchy (top + per-cell library references), not the
macro-of-macros depth zerosoc would have offered. That depth was deemed
unnecessary because `klt extract` is deliberately flat (`docs/cli/extract.md`
→ "Extraction is flat, not hierarchical"), so no downstream verb
(`drc`/`precheck`/`layout-metrics`/`lvs`) has a code path that hierarchy
depth would exercise but this fixture doesn't. **The scgallery/zerosoc
route is therefore not pursued** — a future reader hitting this note should
not re-open the question; if a genuine macro-of-macros hierarchy need
surfaces, file a fresh issue rather than reviving this deferral.
