...
### 4.11 Sequential equivalence checking (Epic #707 Phase 2)

Survey: [`docs/design/sequential-equivalence-survey.md`](../design/sequential-equivalence-survey.md)
([#1290](https://github.com/2AMLogic/klayout-tools/issues/1290), part of
Epic [#707](https://github.com/2AMLogic/klayout-tools/issues/707), PR
[#1295](https://github.com/2AMLogic/klayout-tools/pull/1295)).

| Resource | Provenance & licensing | Informs | Landing spot(s) |
| --- | --- | --- | --- |
| Yosys's own `equiv_make`/`equiv_simple`/`equiv_induct`/`equiv_status` command family — register-correspondence sequential equivalence via temporal induction | Public, open-source tool, part of Yosys itself (no separate install). Live-verified while shipping #1313: SymbiYosys (`sby`, pinned `v0.67`, #1312) implements only `bmc`/`prove`/`cover`/`live`/`prep` modes — there is no `sby mode equiv` this survey's own §4.2 assumed; `equiv_make`/`equiv_induct` are plain built-in Yosys passes, orchestrated directly instead. | Sequential equivalence checking between a pre-P&R and post-route netlist | **Landed** — [`klt equiv`](../cli/equiv.md)'s `"yosys-sequential"` engine (#1313), driven via a two-stage `equiv_make`/`equiv_simple`/`equiv_induct`/`equiv_status` proof plus a bounded-SAT fallback for genuine counterexample extraction. |
| Biere, Cimatti, Clarke, Zhu — "Symbolic Model Checking without BDDs" (bounded model checking), *TACAS*, 1999; Sheeran, Singh, Stålmarck — "Checking Safety Properties Using Induction and a SAT-Solver" (k-induction), *FMCAD*, 2000 | Primary literature, cited by title/author; the technique SymbiYosys's own `mode bmc`/`mode prove` expose. | The harder, BMC/k-induction half of sequential equivalence, for cases plain register correspondence cannot resolve (e.g. a genuine retiming case) | **No landing spot yet** — this survey's own §4.4 stages this behind §4.2 shipping first and finding a real case that needs it; explicitly not proposed as a near-term implementation. |
| This survey's own P&R-transformation-shape finding (§2), grounded in `docs/cli/place-and-route.md`'s real, measured pre-P&R/post-route netlist diff (issue #996) | This repo's own data (a real OpenROAD run: 40 of ~720 instances changed, all CTS/timing-repair buffer insertion or drive-strength resize — zero register-count change). | Which sequential-equivalence technique actually matches this repo's own P&R output | **doc** — this survey's own central finding: register correspondence, not general BMC/k-induction, matches the real P&R transformation shapes observed here, driving §4's staging order. No `klt` code traces to this finding yet — it is a proposal input, not a shipped capability. |
