...
| Resource | Provenance & licensing | Informs | Landing spot(s) |
| --- | --- | --- | --- |
| SymbiYosys (`YosysHQ/sby`) and Yosys's own `equiv_make`/`equiv_simple`/`equiv_induct`/`equiv_status` command family — register-correspondence sequential equivalence via temporal induction | Public, open-source tool; command surface cited from Yosys/SymbiYosys's own documentation — not independently container-verified in this task (no reachable `sby` install; this repo's own CI does not install one today). | Sequential equivalence checking between a pre-P&R and post-route netlist | **No landing spot yet** — this survey's own §4.2/§6 propose a `"yosys-sequential"` `klt equiv` engine as the first follow-on issue; not yet built. |
