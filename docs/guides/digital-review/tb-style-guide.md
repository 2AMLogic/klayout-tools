...
> vocabulary into this repo's own vocabulary; the style content carries
> over unchanged. See [`README.md`](README.md) for when this guide
> applies.

Canonical style guide for **SystemVerilog** testbenches (`*_tb.sv`),
including unit testbenches and testbenches that compile across multiple
hardware configurations. This is the SystemVerilog counterpart of
[`cocotb-tb-style-guide.md`](cocotb-tb-style-guide.md); it does not apply
to Python testbenches — see [`tb-review.md`](tb-review.md)'s opening note
on which testbench kind `klt functional-verification` actually runs.
...
Clock initialized in `initial`, toggled in `always`. Delay before reset
release so parallel `initial` blocks can generate data.
...
Drive and sample synchronous signals through a clocking block. Bare
blocking assigns or reads after `@(posedge clk)` race the RTL Active/NBA
regions; Icarus exposes this, trace dumpers can hide it.
...
Never check registered DUT outputs in the same simulator region that
updates them. RTL flops normally use nonblocking assignments (`<=`), so
values settle in the NBA region after `@(posedge clk)` or `@(posedge
rst)` TB code has run. Same-edge checks can see stale values and falsely
fail.

Use:

- Synchronous interfaces: clocking-block input skew from Section 2.1.
- Simple/async fallback: small settle delay (`#1` in this guide's
  timescale) before reading DUT outputs.
- Reset checks: after the settle delay, confirm reset is still asserted
...
Use open-array parameters where possible so one task handles multiple
data sizes.
...
Continuous monitors turn error signals into test failures. Prefer
edge-triggered monitors to avoid flooding on sustained assertions.
...
End simulation with `$finish`, not `$stop`; `$stop` pauses Icarus and
causes sim timeouts.

Every TB must print a human summary, then a machine-readable verdict
sentinel, then `$finish`. `klt functional-verification` does not invoke
this class of testbench directly (it drives cocotb-based TBs — see
[`cocotb-tb-style-guide.md`](cocotb-tb-style-guide.md)), but any
hand-invoked SV sentinel-driven TB in a canary repo should still follow
this: use the sentinel instead of relying on the simulator's own process
return code, which is unreliable — see "Never trusting an exit code" in
[`docs/cli/functional-verification.md`](../../cli/functional-verification.md)
for the same principle applied to cocotb's own `results.xml`. `[SIM_RESULT]`
is the default shown below; a canary may keep different wording when it
documents that project contract (see [`tb-review.md`](tb-review.md)'s
"Simulation contract" input).
...
1. Randomized vectors - broad input-space coverage for unexpected
   interactions.
...
Never use `$urandom_range()` for large ranges (max > ~2^30). Vivado xsim
can generate values outside the specified range. `$urandom_range()` is
acceptable only for small ranges, e.g. `$urandom_range(255)`.
...
Optionally test garbage in unused bit positions to verify DUT ignores
padding.
...
The DUT instance must be named `dut`; automated coverage derives
hierarchy from the instance name. Canonical naming gives
`<tb_module>.dut.*`, predictable without searching the trace.
...
selection, protection mode, bus interface), use a consistent nesting
order:

1. Outermost: hardware variant (e.g., protection mode - affects
   widths/layouts)
...
- Stall/backpressure re-run phase guarded by bus interface ifdef, if
  applicable.
