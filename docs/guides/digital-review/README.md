...
| Insufficient stimulus diversity / weak coverage | `tb-review.md` #15, `cocotb-tb-review.md` #14 | `klt functional-verification --options.coverage` | `coverage.line_pct` / `.toggle_pct` / `.branch_pct` / `.expr_pct` | Verilator engine only; `null` on Icarus runs. High coverage with few vectors is exactly the false-positive these checks name. |
| Engine-dependent sampling (a DUT output read in the active region) | `cocotb-tb-review.md` #8 | `klt functional-verification` (run twice, once per `engine`) | `status` / `tests[]` compared across the two runs | Issue #1972. No single run detects this — the divergence *is* the signal, so run the same request under `"icarus"` and `"verilator"` and compare verdicts. A green run on one engine is not evidence for the other. |
