...
def test_macros_excludes_std_cell_and_other_libraries(tmp_path):
    ...
    `_fd_ip_` entries (see
    ``test_cells_excludes_fd_ip_hard_macro_library`` in
    ``tests/test_pdk_cells.py``) while `klt pdk macros` now reports them.
    """
    root = tmp_path / "install"
    variant_dir = _make_install(root, "sky130A", assets=("libs_ref",))
...
def test_macros_only_hard_macro_ip_present_no_std_cell(tmp_path):
    ...
    cells_report = pdk_cells.list_cell_libraries(root=str(root))
...
def test_real_ihp_sg13g2_stdcell_resolves_lef_and_cell_library():
    ...
    report = pdk_cells.list_cell_libraries(
