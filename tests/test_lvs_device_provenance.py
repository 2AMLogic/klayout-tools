...
Two tiers, mirroring `tests/test_golden_deck.py`'s split for the DRC side:
...
- **Golden layout->netlist pairs**: a minimal, hand-computed synthetic
  layout per device class (MOSFET, resistor, capacitor -- the three classes
  issue #868 names as the acceptance bar), run through `run_extract`,
  asserting the extracted netlist's device parameters exactly match a value
  computed independently from the deck's own provenance-cited coefficients.
  This validates the model end-to-end, from PDK source citation through to
  netlist output, not just that the field is populated.
"""
