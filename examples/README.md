...
- [`characterize/`](characterize/) — a `klt characterize` worked example: a
  synthetic 2-input NAND characterized into an NLDM Liberty (`.lib`) timing
  model at one PVT corner, needing only `ngspice` (issue #2502). See its
  README.
- [`critical-net-mom-fidelity/`](critical-net-mom-fidelity/) — measurement
  fixture grading `klt extract --critical-net`'s lateral-coupling capacitance
  against `klt mom`'s field solver (issue #978). See its README.
...
- [`yield-sensitivity/`](yield-sensitivity/) — a synthetic, seeded 200-sample
  campaign for `klt yield-sensitivity` (issue #923); `generate.py`
  regenerates. See its README.
