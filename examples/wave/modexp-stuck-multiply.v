// FIXTURE ONLY -- deliberately mutated copy of
// `examples/functional-verification/modexp.v` (the sky130-modexp canary's
// own RSA-style modular-exponentiation RTL), used exclusively to reproduce
// a real cocotb failure for
// `docs/guides/waveform-first-debugging.md`. This is not the canary's real
// RTL and ships no bug of its own -- see that guide for the one-line
// mutation (`S_MM_RUN`'s loop-terminal sentinel) and how `klt wave query`
// diagnoses it without ever reading this file.
//
    ...
    if (!rst_n) begin
      ...
      case (state)
        ...
        S_MM_RUN: begin
          ...
          mm_a <= mm_a << 1;
          // FIXTURE MUTATION (see file header): the correct terminal
          // sentinel is a CW-bit encoding of decimal 1
          // (`{{(CW-1){1'b0}}, 1'b1}`) -- j counts WIDTH..1, one interleaved
          // step per exponent bit. This all-ones sentinel is only reached
          // after j underflows past zero, so every square/multiply pass
          // runs WIDTH+2 cycles instead of WIDTH.
          if (j == {CW{1'b1}}) begin
            state <= S_AFTER;
          end else begin
            j <= j - 1'b1;
