...
- **If `loom:reviewing` is no longer in `CURRENT_LABELS`**: someone else
  already resolved this claim between your read and now — do nothing further,
  re-run the Primary Queue from step 1.
- **If `loom:reviewing` is still present** (the normal case here) **and
