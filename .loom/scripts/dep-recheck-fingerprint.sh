...
# encoding change, emitted by `dep-recheck` and `operator-premise` only (not
# `named-dependency`, which has no consumer that needs this guard yet).
# Embed it alongside CONCLUSION_HASH in every marker (e.g.
...
#   resync-churned script in this directory) can silently reshape what
#   CONCLUSION_HASH means for every open issue at once, and every deployed
#   agent misreads "the recipe changed" as "the dependency changed" — the
#   exact fleet-wide false-CHANGED incident (#1544) that on 2026-09-06
#   spammed #528/#527/#56/#55 with spurious comments.
#
...
#   in the resync commit; do not assume upstream did it for you. This exact
#   guard was silently stripped by an unpinned resync once already (#1546,
#   commit 090cd9c) less than an hour after it was first added (#1544/#1545)
#   — both this script and its test file are now pinned in
#   `.loom/resync-ignore` specifically to prevent a third occurrence.
