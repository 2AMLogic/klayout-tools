...
### Dispatch race: a claim taken on approval already reverted (#1710, 2026-09-12)

The passes above all answer questions about a claim's *aliveness* (is the
claimant process still running, is the reviewed tree still the head, is a
survivor still occupying a slot). This is a different failure shape entirely:
a **live**, self-correcting claim taken on approval that was already stale
*before the claim was even made*.

**What was observed.** On issue #1697 (2026-09-12), the label timeline was:

```
11:00:03Z  labeled    loom:evaluating
11:00:09Z  unlabeled  loom:hermit
11:00:09Z  unlabeled  loom:evaluating
11:00:09Z  labeled    loom:issue        <- Champion promotion (all 8 criteria passed)
11:01:48Z  unlabeled  loom:issue        <- Champion reverts (Tier-3 backlog-cap counting bug, unrelated to #1697's merit)
11:01:48Z  labeled    loom:hermit
11:03:29Z  labeled    loom:building     <- daemon dispatch claim, 101s AFTER the revert
```

A daemon-dispatched `/loom:sweep 1697 --claim-owned 1697` child observed
`loom:building` on an issue that, by the time the child actually started, no
longer carried `loom:issue` or `loom:curated`. The daemon's dispatch decision
appears to have been made during the brief `loom:issue` window
(11:00:09Z-11:01:48Z) and executed ~101s later, without re-checking current
label state at execution time.

**Why the reconciliation passes above do not catch it.** Every pass in this
section — `claim_reconciliation::forge::reconcile_workspace`, the sweep
journal, `reconcile_pr_verdicts`, the startup adoption seed — detects a *dead*
claim: a claimant process that exited, a tree that moved out from under a
verdict, a survivor the registry lost track of. This race produces none of
those symptoms. The claim here is fully *live* (a real sweep child, running
normally) and *self-correcting* (see below) — it was simply taken on a
promotion that had already been reverted before the claiming dispatch ever
ran. No existing pass asks "was the label that justified this claim still
current at claim time?", because every other claim path here writes its own
claim at the moment it checks the label; only daemon dispatch has a gap
between the label check and the label write.

**Why the fix cannot land in this repo.** `.claude/commands/loom/sweep.md`'s
Step 1a self-claim check (added by #4111) deliberately forbids the receiving
sweep session from second-guessing its own `--claim-owned` /
`LOOM_SWEEP_CLAIM_OWNED` claim based on label timing, specifically to prevent
a false-positive "am I racing another worker?" investigation. Reopening that
check to add stale-approval detection would reintroduce the #4111 failure
mode, so that surface is off-limits here (and this issue's acceptance
criteria explicitly forbid touching it). The durable fix instead belongs in
`loom-daemon/src/sweep_registry/dispatch.rs`'s `spawn_child`, in the
**upstream `rjwalters/loom` repo** (not vendored in this repo, confirmed via
`git ls-tree -r origin/main --name-only | grep -i dispatch.rs` returning
nothing) — it should re-check that the target issue still carries
`loom:issue` (or `loom:curated`) immediately before flipping it to
`loom:building`, and skip the dispatch (returning the issue to normal
triage) if the label is already gone by the time the check runs. This should
be filed as an issue against `rjwalters/loom` directly; it is out of scope
for this issue's (#1710's) acceptance criteria, since this repo has no
config-level or role-file-level mitigation available for a dispatch-time
timing gap.

**Impact: self-correcting, not silent bad output.** On the concrete #1697
instance, the dispatched sweep child itself detected the reverted approval
later in its own pipeline (not via the `--claim-owned` pre-flight) and
released the `loom:building` claim with an explanatory comment. The observed
impact was a transient false `loom:building` label window (~101s+, until the
child's own later step caught it) and one wasted dispatch cycle — **not**
unapproved work actually getting built. #1697 itself closed normally
afterward.
