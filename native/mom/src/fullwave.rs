...
//! `compute_inductance` already established (`geometry::classify_bars`:
//! every box, of every conductor, is individually bar-shaped, and a
//! multi-box conductor's own boxes share one current-flow axis and axial
//! extent -- see docs/cli/mom.md's "PEEC inductance/resistance" section for
//! why). A multi-box conductor's boxes are combined into one equivalent thin
//! wire (`geometry::classify_full_wave_bars` sums each box's cross-sectional
//! area and area-weights their centroids) before this module's own
//! per-conductor mesh below. Since issue #1842 this scope no longer includes
//! the old shared-current-flow-axis and shared-axial-span restrictions
//! *across* conductors: they may run along different axes and over different
//! spans, exactly as in PEEC's static solve. Where PEEC's static solve
//! (`peec.rs`) discretises each conductor's
...
mod tests {
    ...
    fn multi_box_conductor_combines_into_one_equivalent_wire() {
        ...
        // (identical in both boxes). The merged geometry carries the
        // centroid as the equivalent wire's 3-D centreline endpoints (issue
        // #1842's arbitrary-orientation shape), running the full [0, 100]um
        // along x (both boxes' longest extent) at that transverse centroid.
        assert!(
            (geom.area_um2 - 12.0).abs() < 1e-9,
            "unexpected area: {}",
            geom.area_um2
        );
        for (label, point) in [("start", geom.start_um), ("end", geom.end_um)] {
            let axial = if label == "start" { 0.0 } else { 100.0 };
            assert!(
                (point[0] - axial).abs() < 1e-9,
                "unexpected {label} x: {}",
                point[0]
            );
            assert!(
                (point[1] - 100.0 / 12.0).abs() < 1e-9,
                "unexpected {label} y (area-weighted centroid): {}",
                point[1]
            );
            assert!(
                (point[2] - 1.0).abs() < 1e-9,
                "unexpected {label} z (area-weighted centroid): {}",
                point[2]
            );
        }
    }
