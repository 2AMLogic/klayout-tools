// Ported from boldaxolotl/booley, crates/bwave/src/parser.rs
...
//
// Modifications from upstream: none at port time -- ported verbatim
// (including the two clippy::manual_pattern_char_comparison sites and the
// one clippy::unwrap_or_default site below, left exactly as upstream wrote
// them rather than "fixed" out from under the verbatim-port claim -- see
// the crate-level `#![allow(...)]` in lib.rs). `klt wave build`
// (native/wave/src/build.rs) drives the `VcdHandler` trait defined here via
// `parse_streaming` for single-threaded streaming VCD ingest. See /NOTICE
// and docs/design/waveform-query-contract-spike.md section 12b for the
// attribution convention this header follows.
