...
| `nldm.rs` | Thin re-export of the shared `native/nldm-interp` crate's bilinear NLDM interpolation (issue #2272; was a verbatim fork of `native/statime/src/nldm.rs`) — used for a single representative-operating-point delay estimate, not a path-based STA graph. |
