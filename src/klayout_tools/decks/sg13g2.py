...
# `ParasiticsDeck`) -- all seven declared metal levels (issue #1281,
# extending #1277's initial Metal1/Metal2 slice). Epic #711 Phase 3b
...
# (issue #1277's own repro). Issue #1277 populated Metal1/Metal2; this
# entry (#1281) fills in the remaining Metal3-TopMetal2 levels
# (`EXTRACTION_DECK.metals` has seven levels total as of PR #1247/#1258),
# so `PARASITICS.metals` now covers the full declared stack.
#
...
# lines 189-502, auto-generated by the upstream `capiche` field-solver tool
# per that file's own inline comment) is the same corner sky130.py/
# gf180mcu.py's own "nominal `variants` block" convention selects -- and the
# only one of the file's three near-identical per-corner `allm3`-`allm7`
# blocks (`variants ()` at 189-502, `variants (hrhc),(lrhc)` at 503-812,
# `variants (hrlc),(lrlc)` at 813-1120) used below; picking a different
# corner block would produce plausible-looking but wrong numbers. Values
...
#
# Metal3-TopMetal2's `sheet_res_ohm_sq` (issue #1281) is transcribed
# **directly** from this same file's nominal `resist (),(lvs)` block (lines
# 84-88) -- unlike Metal1/Metal2, `EXTRACTION_DECK.resistors` does not (yet)
# declare `res_metal3`..`res_topmetal2` (see that table's own module
# comment), so there is no already-curated value to reuse. Cross-validated
# against `libs.tech/klayout/tech/lvs/rule_decks/res_extraction.lvs` lines
# 67-71 (`RSH_RES_METAL3`..`RSH_RES_TOPMETAL2`), which agree exactly --
# two independent upstream files, same corner.
PARASITICS = ParasiticsDeck(
    # No diffusion/poly role curated (out of scope -- issue #1277's own
    # scoping, unchanged by #1281's metal-stack extension).
    diffusion=None,
    poly=None,
    metals=(
        ...
        # Metal3 (`EXTRACTION_DECK.metals[2]`). `defaultareacap allm3
        # metal3 11.994` / `defaultperimeter allm3 metal3 31.352`
        # (`ihp-sg13g2-extract.tech` lines 270-271). `sheet_res_ohm_sq`
        # transcribed from `resist (allm3)/metal3 88` (line 84; see module
        # comment above -- no `res_metal3` curated yet to reuse).
        LayerRC(
            sheet_res_ohm_sq=0.088, cap_area_ff_um2=0.011994, cap_perim_ff_um=0.031352
        ),
        # Metal4 (`EXTRACTION_DECK.metals[3]`). `defaultareacap allm4
        # metal4 8.948` / `defaultperimeter allm4 metal4 29.083`
        # (`ihp-sg13g2-extract.tech` lines 306-307). `sheet_res_ohm_sq`
        # transcribed from `resist (allm4)/metal4 88` (line 85).
        LayerRC(
            sheet_res_ohm_sq=0.088, cap_area_ff_um2=0.008948, cap_perim_ff_um=0.029083
        ),
        # Metal5 (`EXTRACTION_DECK.metals[4]`). `defaultareacap allm5
        # metal5 7.136` / `defaultperimeter allm5 metal5 27.527`
        # (`ihp-sg13g2-extract.tech` lines 347-348). `sheet_res_ohm_sq`
        # transcribed from `resist (allm5)/metal5 88` (line 86).
        LayerRC(
            sheet_res_ohm_sq=0.088, cap_area_ff_um2=0.007136, cap_perim_ff_um=0.027527
        ),
        # TopMetal1 (`EXTRACTION_DECK.metals[5]`, `allm6`/`metal6` in the
        # tech file). `defaultareacap allm6 metal6 5.649` /
        # `defaultperimeter allm6 metal6 37.383` (`ihp-sg13g2-extract.tech`
        # lines 393-394). `sheet_res_ohm_sq` transcribed from
        # `resist (allm6)/metal6 18` (line 87).
        LayerRC(
            sheet_res_ohm_sq=0.018, cap_area_ff_um2=0.005649, cap_perim_ff_um=0.037383
        ),
        # TopMetal2 (`EXTRACTION_DECK.metals[6]`, `allm7`/`metal7` in the
        # tech file). `defaultareacap allm7 metal7 3.233` /
        # `defaultperimeter allm7 metal7 31.175` (`ihp-sg13g2-extract.tech`
        # lines 444-445). `sheet_res_ohm_sq` transcribed from
        # `resist (allm7)/metal7 11` (line 88).
        LayerRC(
            sheet_res_ohm_sq=0.011, cap_area_ff_um2=0.003233, cap_perim_ff_um=0.031175
        ),
    ),
    # `defaultoverlap` vertical plate-coupling coefficients for each
    # adjacent metal-level pair, index-aligned with `metals` above
    # (`metal_overlaps[i]` = overlap between `metals[i]` and `metals[i+1]`):
    metal_overlaps=(
        # Metal1<->Metal2: `defaultoverlap allm2 metal2 allm1 metal1
        # 67.225` (`ihp-sg13g2-extract.tech` line 264, under the
        # `# metal2->metal1` nominal-corner block).
        0.067225,
        # Metal2<->Metal3: `defaultoverlap allm3 metal3 allm2 metal2
        # 67.225` (line 300, `# metal3->metal2`).
        0.067225,
        # Metal3<->Metal4: `defaultoverlap allm4 metal4 allm3 metal3
        # 67.225` (line 341, `# metal4->metal3`).
        0.067225,
        # Metal4<->Metal5: `defaultoverlap allm5 metal5 allm4 metal4
        # 67.225` (line 387, `# metal5->metal4`).
        0.067225,
        # Metal5<->TopMetal1: `defaultoverlap allm6 metal6 allm5 metal5
        # 42.708` (line 438, `# topmetal1->metal5`).
        0.042708,
        # TopMetal1<->TopMetal2: `defaultoverlap allm7 metal7 allm6 metal6
        # 12.965` (line 494, `# topmetal2->topmetal1`).
        0.012965,
    ),
)
