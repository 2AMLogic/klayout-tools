...
Ported from:
    ...
    Project:  boldaxolotl/booley -- https://github.com/boldaxolotl/booley
    File:     src/booley/dev_support/mutation_variants.py
    Branch:   main
    Fetched:  2026-09-09 (raw.githubusercontent.com)
    License:  Apache License, Version 2.0
              https://www.apache.org/licenses/LICENSE-2.0

Copyright (c) the boldaxolotl/booley authors. Licensed under the Apache
...
limitations under the License. A copy of the License is available at the URL
above; it is not reproduced in full here (this repo carries no ``NOTICE``
file and neither does the upstream project -- see
``docs/design/mutation-testing-spike.md``, "Open questions" ->
"Attribution mechanics").

Changes from upstream: none. This file is a byte-exact port of the fetched
source above (only this header block, the module docstring's continuation
above it, and this "Changes from upstream" note were added; every line of
code below is unmodified). ``mutation_lock.py`` (upstream's warm-cache
machinery for a *live LLM creator agent's* repeated proposal-generation
cost) was deliberately **not** ported -- see
``docs/design/mutation-testing-spike.md`` section 1a for why it does not
apply to ``klt``'s "agent authors a proposals.json once, klt executes it
deterministically" model. Owned by ``klayout_tools.functional_verification``
(issue #1592), which drives ``MutationVariantPlan`` directly rather than
through booley's own (not ported) ``mutation_tester.py`` orchestration --
see that module for the ``klt``-specific per-proposal rejection posture
(a proposal naming a file outside scope is rejected individually, not a
whole-document failure) and the ``klt``-level per-mutant subprocess timeout,
neither of which exists in this file.
"""
