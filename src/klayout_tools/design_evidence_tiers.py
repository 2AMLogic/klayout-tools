...
#: Default location of the source doc, resolved relative to this file's
#: install location the same way ``kb.py``'s ``DEFAULT_KB_ROOT`` resolves
#: the ``kb/`` directory: ``src/klayout_tools/design_evidence_tiers.py`` ->
#: parent (``src/klayout_tools``) -> parent (``src``) -> parent (repo root).
...
def parse_tier_doc(path: str | Path | None = None) -> dict[str, Any]:
    """Parse ``docs/design-evidence-tiers.md`` (or ``path``, if given) into::
    ...
    try:
        ...
        raise DesignEvidenceTiersError(
            f"could not read design-evidence-tiers doc at '{doc_path}': {exc}"
        ) from exc
