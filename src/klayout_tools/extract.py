...
implements (its section 2a, ``klt extract``). Default scope:
**schematic-equivalent** extraction only -- devices and connectivity, no
parasitic R/C on the interconnect.

Parasitics (issue #217) are available behind the opt-in ``--parasitics``
flag, per the interface decision recorded in that spike's "Addendum (#216)":
a first-order lumped R + C-to-ground reduction over the extracted nets,
implemented in :mod:`klayout_tools.parasitics` (read that module's docstring
for the engine survey behind the "build it, don't wrap it" call, the model,
and its stated limits). The flag is strictly additive -- with it omitted,
this module's output is byte-identical to what it produced before #217.
...
#: Device classes generated by the `--parasitics` pass (see
#: `klayout_tools.parasitics`). Kept OUT of the response's `devices[]` /
#: `device_counts` / `device_count` view, whose documented meaning ("one entry
#: per extracted device") is unchanged by the flag -- parasitic elements are
#: summarised in the separate, additive `parasitics` block instead.
_PARASITIC_CLASSES = frozenset({RESISTOR_CLASS, CAPACITOR_CLASS})
...
def run_extract(
    ...
    ``parasitics`` (the ``--parasitics`` flag, off by default) additionally
    runs the first-order lumped RC reduction in
    :mod:`klayout_tools.parasitics` over the extracted nets, adding parasitic
    ``R``/``C`` primitives to the *same* written netlist and a ``parasitics``
    block (plus per-net ``resistance_ohm``/``capacitance_ff``) to the
    response. Strictly additive: with the flag omitted, every field below is
    exactly what it was before issue #217, and the written SPICE is unchanged.
    ...
    section 2a)::

        {
            ...
            "parasitics": {  # only when `parasitics=True`, else None
    ...
    netlist, top_cell_name, dbu_um, warnings, parasitics_result = (
        extract_netlist_from_layout(path, deck_name, top=top, parasitics=parasitics)
    )
    ...
    writer = (
        kdb.NetlistSpiceWriter(
            create_model_binding_delegate(
                model_class_to_subckt or {},
                _PARASITIC_CLASSES if parasitics_result is not None else frozenset(),
            )
        )
        if model_class_to_subckt is not None or parasitics_result is not None
    ...
    if circuit is not None:
        ...
        nets = _describe_nets(circuit, parasitics_result)
    else:
    ...
    result["parasitics"] = (
        parasitics_result.summary() if parasitics_result is not None else None
    )
...
def extract_netlist_from_layout(
    path: str, deck_name: str, top: str | None = None, parasitics: bool = False
) -> tuple[kdb.Netlist, str, float, list[str], ParasiticsResult | None]:
    """Core extraction: read ``path``, resolve ``deck_name`` and the top
    cell, and run flat device + connectivity extraction. Returns
    ``(netlist, top_cell_name, dbu_um, warnings, parasitics_result)``, where
    the last element is ``None`` unless ``parasitics`` is set.
    ...
    round-trip through a written SPICE file just to compare it. ``klt lvs``
    never passes ``parasitics``: LVS compares *schematic equivalence*, and a
    parasitic-annotated netlist would not be isomorphic to any schematic
    reference (see ``docs/cli/extract.md`` -> "Parasitics and LVS").
    ...
    parasitics_tech = get_parasitics_tech(deck_name) if parasitics else None
    ...
    netlist, warnings, parasitics_result = _extract_netlist(
        layout, top_cell, deck, parasitics_tech
    )
    return netlist, top_cell.name, layout.dbu, warnings, parasitics_result
...
def _extract_netlist(
    ...
    top_cell: kdb.Cell,
    deck: ExtractionDeck,
    parasitics_tech: ParasiticTech | None = None,
) -> tuple[kdb.Netlist, list[str], ParasiticsResult | None]:
    ...
    Returns ``(netlist, warnings, parasitics_result)``. ``warnings`` is built
    ...
    ``parasitics_result`` is ``None`` unless ``parasitics_tech`` is given, in
    which case the lumped RC pass has already been applied to the returned
    netlist (it runs here, not in the caller, because it needs the live
    ``LayoutToNetlist`` for per-net geometry -- which this function otherwise
    lets go out of scope).
    """
    import klayout.db as kdb
    ...
    # Parasitics run *after* `purge()` (so the pass never has to reason about
    # nets that are about to disappear) and *before* `dup()` (so the live
    # `l2n` is still available for per-net shape lookup).
    parasitics_result: ParasiticsResult | None = None
    if parasitics_tech is not None:
        parasitics_result = annotate_parasitics(
            l2n,
            netlist,
            top_cell.name,
            build_parasitic_layers(
        ...
        warnings.extend(parasitics_result.warnings)
    ...
    return netlist.dup(), warnings, parasitics_result
...
def _describe_devices(
    ...
    Parasitic R/C elements (``--parasitics``) are skipped: ``devices[]`` means
    "one entry per extracted *device*" with or without the flag, and the
    generated elements are summarised in the response's ``parasitics`` block
    instead (see ``_PARASITIC_CLASSES`` and ``docs/cli/extract.md``).
    """
    ...
    for device in circuit.each_device():
        ...
        if class_name in _PARASITIC_CLASSES:
            continue
...
def _describe_nets(
    circuit: kdb.Circuit, parasitics_result: ParasiticsResult | None = None
) -> list[dict[str, Any]]:
    """Build the response's ``nets[]`` array.

    The three documented fields (``name``/``pin``/``device_count``) mean
    exactly what they meant before ``--parasitics`` existed: the internal RC
    nodes the parasitics pass created are not reported as nets, and
    ``device_count`` counts only real extracted devices, never the generated
    R/C elements.

    When ``parasitics_result`` is given, each entry additionally carries the
    net's modelled ``resistance_ohm``/``capacitance_ff`` (``0.0`` for a net
    with no geometry on any curated parasitic layer) -- new, independently
    optional fields, absent entirely when the flag is off.
    """
    pin_nets = {pin.expanded_name() for pin in _each_pin_net(circuit)}
    internal_nets = (
        parasitics_result.internal_net_names if parasitics_result is not None else set()
    )

    nets: list[dict[str, Any]] = []
    for net in circuit.each_net():
        name = net.expanded_name()
        if name in internal_nets:
            continue
        entry: dict[str, Any] = {
            "name": name,
            ...
            "device_count": sum(
                1
                for terminal in net.each_terminal()
                if terminal.device().device_class().name not in _PARASITIC_CLASSES
            ),
        }
        if parasitics_result is not None:
            parasitics = parasitics_result.per_net.get(name)
            entry["resistance_ohm"] = parasitics.resistance_ohm if parasitics else 0.0
            entry["capacitance_ff"] = parasitics.capacitance_ff if parasitics else 0.0
        nets.append(entry)
